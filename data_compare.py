...
class DataCompare:
    def __init__(self, yaml_file, data_source_dsn, data_type,table_name,partition_value):
        ...
        if table_name:
            self.lock = pid.PidFile(pidname="%s.LOCK" % APP_NAME+"_"+data_source_dsn+"_"+data_type+"_"+table_name,
    ...
    def generate_sql_for_fact(self, table_name,  column_list, partition_key):
        ...
        with OXDB('SNOWFLAKE') as oxdb:
            self.logger.info(self.data_source_dsn+','+table_name+','+self.table_schema+','+partition_key)
            max_value = oxdb.get_executed_cursor(self.config['GET_MAX_VALUE'], (self.data_source_dsn,table_name, self.table_schema, partition_key)).fetchall()
        ...
        if self.partition_value:
            ...
            with OXDB('SNOWFLAKE') as oxdb:
                ...
                table_name, self.table_schema, partition_key)).fetchall()
        ...
        if table_name:
            with OXDB('SNOWFLAKE') as oxdb:
                if self.partition_value:
                    ...
                    oxdb.execute(self.config['DELETE_EXISTING_TABLE_DATA'],
                                 (self.data_source_dsn, table_name))

                oxdb.commit()

        else:
            """oxdb = OXDB('SNOWFLAKE')
                populate_table_list = oxdb.get_executed_cursor(self.config['FIND_HOURLY_TABLE'])
                ...
                oxdb = OXDB('SNOWFLAKE')
                populate_table_list = oxdb.get_executed_cursor(self.config['FIND_DAILY_TABLE'])
            ...
            #if self.data_source_dsn=='SNOWFLAKE':
                #with OXDB('SNOWFLAKE') as oxdb:
                #    oxdb.get_executed_cursor(self.config['DELETE_VERTICA_MISMATCH'])
                #    oxdb.get_executed_cursor(self.config['DELETE_IMPALA_MISMATCH'])
                #    oxdb.commit()
        try:
